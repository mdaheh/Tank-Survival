...
**Создано:** 2026-09-22 · **Автор аудита:** Cline (AI-агент, диалог-сессия) · **Статус:** анализ завершён, код ещё не переписывался

**Стек:** Unity 6000.5.9f1 · URP 17.5.0 · Input System 1.20.0 · uGUI + TextMeshPro · Post Processing 3.5.4 · цель — WebGL

> **Зачем этот файл:** это снимок полного архитектурного аудита проекта. Если контекст диалога потерян или сменилась модель ИИ — читай этот файл вместо повторного исследования проекта. Раздел 8 — живой чек-лист реализации, его нужно обновлять по мере работы.
>
> **Ключевые ограничения** (из `.clinerules`): производительность WebGL — главный приоритет (пулы, ноль аллокаций в кадре, статические батчи); сцена **никогда** не перезагружается между раундами; на экране может быть до нескольких сотен врагов и снарядов; в `Update/LateUpdate` — никаких дорогих операций; каждое архитектурное решение — с Pros/Cons и влиянием на WebGL.
>
> **Суть геймдизайна** (из `game_design.md`): модульный танк «Шасси + Турель»; турель автоматически атакует ближайшего врага; игрок управляет только перемещением и выбором улучшений; до 4 типов врагов, идут волнами (число волн зависит от сложности — 3 сложности); при level-up выбор 1 из 3 случайных улучшений; между забегами сохраняется только глобальный счётчик убийств (+ разблокировки); все числовые параметры — через ScriptableObject, новый контент — без изменения кода.
...
| Менеджер волн под ГД | ⚠️ заглушка | Нет `WaveData` (состав, соотношение типов, частота), нет данных о 4 типах врагов |
...
| `Health` | HP-состояние без UI; полоски — отдельные `HealthBar`, слушают события |
...
## 8. Чек-лист реализации (обновлять по мере работы)

- [ ] Ф0: турель монтируется по ссылке (не `Find("TurretPos")`)
- [ ] Ф0: префаб врага имеет логику (движение + HP + смерть)
- [ ] Ф0: `PauseMenu` без `LoadScene`
- [ ] Ф0: NPE в `ShellExplosion` устранён (`IDamageable` / null-check)
- [ ] Ф1: `RunContext` + `StatBlock` + применение апгрейдов к стейту
- [ ] Ф1: `DamageSystem` + `IDamageable`
- [ ] Ф1: `EnemyRegistry`
- [ ] Ф1: пул врагов и снарядов (prewarm)
- [ ] Ф1: `EnemyData` SO
- [ ] Ф2: `WaveData` + `WaveController` + `SpawnDirector`
- [ ] Ф2: XP из `EnemyData`; level-up через `LevelSystem`
- [ ] Ф3: `ProgressService` + `SaveStore` (PlayerPrefs, save в конце раунда)
- [ ] Ф3: Ангар как отдельный экран + сохранение выбора
- [ ] Ф3: UI разбит на экраны; HUD по событиям; TMP-кэш значений
- [ ] Ф4: кинематика врагов; батчи/instancing; лимит SFX; профилирование
